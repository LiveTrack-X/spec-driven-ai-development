# Open Implementation Items

Status: Active
Scope: Current implementation backlog only

## Current Priority

1. Keep review findings current.
2. Finish the smallest useful active SPEC slice.
3. Keep code, tests, and docs aligned.
4. Keep release or production-readiness gates explicit when applicable.

## Active Work

- [ ] Define the first active SPEC slice with evidence needed and completion condition.

## Future / Deferred

- Record ideas here only when they are not active implementation work yet.
- If this file becomes a long journal, move old material to archive/history and
  keep this file focused on current active work.

## Release / Production Readiness

- [ ] Define release or production evidence if this project will be shipped or deployed.
