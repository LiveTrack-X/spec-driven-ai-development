...
詳しい FAQ と状況別の説明は [docs/user-guide.md](docs/user-guide.md) を参照してください。
短い判断基準は次のとおりです。
...
- Q. AI が頻繁に承認を求めすぎる。
  A. 承認済みの packet 境界が明確なら `Level 2 Work Packet Autonomy` に上げます。
  ただし release、migration、destructive action、user data、auth、money、
  security、rollback gate では Owner approval を維持します。
- Q. AI が先に進みすぎる。
  A. `Level 1 Unit Autonomy` に下げ、work packet を狭めます。1 unit だけを
  終えた後、変更ファイル、確認証拠、制限、Owner decision needed を報告させます。
- Q. AI が "done" とだけ言う。
...
- [docs/user-guide.md](docs/user-guide.md): 状況別ユーザーガイドと FAQ
