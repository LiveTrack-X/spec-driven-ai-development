...
ステータス: `3.2.2` 安定ドキュメント/パッケージリリース。
...
1. Current beats historical — 現在有効な情報を過去の記録より優先します。
2. Evidence beats confidence — AI の自信より観測可能な evidence を優先します。
3. Active beats interesting — 興味深い案より承認済みの active scope を優先します。
4. Owner decision beats AI momentum — AI の勢いより Owner の決定を優先します。
5. Repeated pain becomes a rule — 繰り返す問題は rule、test、validation に変えます。

Compression first. Gates stay real.

ここで **Current** は最新の file や時刻ではなく、現在適用される Owner 指示と明示的に active な
repository authority を意味します。Owner が方向を変えたら、agent は影響する旧作業を止め、
Plan -> Route に戻って変更を記録した後に stateful implementation を再開します。Rule 5 は単に
rule を増やすことではなく、root cause から最小の enforceable change と regression evidence を
作り、後で Keep/Refine/Merge/Retire する feedback loop です。

詳しい意味は [Implicit Rules Made Explicit](docs/implicit-rules.md) を参照してください。
