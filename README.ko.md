...
자세한 FAQ와 상황별 설명은 [docs/user-guide.md](docs/user-guide.md)를 보세요.
빠른 기준은 다음입니다.
...
- Q. AI가 너무 자주 승인 요청을 해요.
  A. 승인된 packet 경계가 분명하다면 `Level 2 Work Packet Autonomy`로 올리세요.
  다만 release, migration, destructive action, user data, auth, money,
  security, rollback gate는 오너 승인을 유지해야 합니다.
- Q. AI가 너무 앞서 나가요.
  A. `Level 1 Unit Autonomy`로 낮추고 work packet을 더 좁히세요. 한 unit만
  끝낸 뒤 변경 파일, 확인 증거, 한계, 오너 결정 필요사항을 보고하게 하세요.
- Q. AI가 "done"이라고만 말해요.
...
- [docs/user-guide.md](docs/user-guide.md): 상황별 사용자 가이드와 FAQ
