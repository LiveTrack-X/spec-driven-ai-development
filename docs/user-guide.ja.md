...
現在適用される Owner 指示は以前の方向より優先されます。方向が変わったら影響を受ける
作業を止め、Plan -> Route に戻って SPEC/state を整合させます。明確な命令文が許可するのは
指定された action と boundary だけです。質問、仮定、引用、否定、review/reference-only
request は、そこで言及された action を許可しません。
...
現在の Owner が直接指定した input は、古い `routed_docs` に無くても request の範囲内で
確認できます。採用した場合は stateful implementation 前に authority と route を更新します。
...
`docs/sdad/handoffs/YYYY-MM-DD-HNNNN-topic.md` に作成します。`HNNNN` は同じ日付内でのみ
増加し、新しい日付の最初は `H0001` に戻ります。別の日付で同じ `HNNNN` があり得るため
完全な path で参照します。既存の番号なし handoff は維持し、現在性は `current_handoff`
だけで決定します。
...
後の制限、取消し、撤回は影響する authorization を直ちに終了します。代替 packet なしで
取り消された場合は `deferred` とし、resume trigger を明示的な Owner 再有効化に固定し、
自動再開しません。
...
です。ファイルの存在、名前、日付、status だけでは SPEC authority は生まれません。
Owner が採用・反映・実装を指示した場合は現在の change request として影響する作業を止め、
active boundary と整合させます。review・compare・explain・draft/reference-only request は
read-only で、単に発見された SPEC にも自動 authority はありません。同じ未完了 acceptance
boundary なら同じ packet を amend できますが、重要な変更または accepted packet 後の変更は
新しい packet ID と validation を使います。
