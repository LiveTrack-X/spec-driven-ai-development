...
`sdad doctor` remains checkout-only in 3.2.2. Adapter and no-clone installation
...
`--require-version 3.2.2` proves only that the invoked program reports the exact
...
3.2.2` when a repeatable local command is needed.
...
The v3.2.2 local Windows release gate ran 443 tests, with three
...
The executable no-clone instructions are pinned to the stable v3.2.2 baseline
at full 40-character commit SHA
`adfd40afd4e1d3fcaba64cc3f5be936c5feb51fd`. Each downloaded adapter is verified
