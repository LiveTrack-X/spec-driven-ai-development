...
`sdad doctor` remains checkout-only in 3.2.3. Adapter and no-clone installation
...
`--require-version 3.2.3` proves only that the invoked program reports the exact
...
3.2.3` when a repeatable local command is needed.
...
The v3.2.3 local Windows release gate ran 492 tests, with three
...
The executable no-clone instructions are pinned to the stable v3.2.3 baseline
at full 40-character commit SHA
`04a3e24569758237b702c5a2fbc6094a186d6cb0`. Each downloaded adapter is verified
