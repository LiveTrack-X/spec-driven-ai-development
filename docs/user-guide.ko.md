...
`docs/sdad/handoffs/YYYY-MM-DD-HNNNN-topic.md`에 만듭니다. `HNNNN`은 기기
시각이 아니라 저장소의 논리 순서이고 날짜는 설명용입니다. 기존 무번호
handoff는 유지하며 현재성은 `current_handoff`만 결정합니다.
...
추가되거나 충돌하는 SPEC은 해당 범위가 그 진입점에 통합되거나 packet transaction으로
pointer가 바뀔 때까지 proposal입니다. accepted packet 뒤의 중대한 변경은 과거를
덮어쓰지 않고 새 packet ID와 새 validation을 사용합니다.
